...
some specific course run. This needs exists for reasons related to upstream
...
Decision
========
* We'll expose a model via Django Admin that allows staff users to force redemption
  for a given learner, into a particular course, under a particular subsidy access policy.
...
where the desired content is not discoverable to the learner (and therefor not enrollable).
