...
from enterprise_access.apps.customer_billing.models import CheckoutIntent, SelfServiceSubscriptionRenewal, StripeEventData, StripeEventSummary
...
class TestGetCreateSubscriptionPlanRenewalStep(TestCase):
    ...
    def test_process_input_creates_renewal_record(self, mock_create_renewal):
        ...
        mock_accumulated_output.create_trial_subscription_plan_output.uuid = mock_renewal_response['prior_subscription_plan']
        mock_accumulated_output.create_trial_subscription_plan_output.expiration_date = mock.Mock()
        
        mock_accumulated_output.create_first_paid_subscription_plan_output = mock.Mock()
        mock_accumulated_output.create_first_paid_subscription_plan_output.uuid = mock_renewal_response['renewed_subscription_plan']
    ...
    def test_process_input_with_existing_renewal_record(self, mock_create_renewal):
        ...
        self.assertEqual(renewal_record.stripe_subscription_id, 'sub_existing_123')

    @mock.patch.object(LicenseManagerApiClient, 'create_subscription_plan_renewal')
    def test_process_input_without_stripe_subscription_id(self, mock_create_renewal):
        """Test creating renewal record when no StripeEventSummary exists yet."""
        mock_renewal_response = {
            'id': 456,
            'title': 'Test Renewal Plan',
            'created': '2024-01-15T10:30:00Z',
        ...
        # Verify a SelfServiceSubscriptionRenewal record was created with empty stripe_subscription_id
    ...
    def test_process_input_license_manager_error(self, mock_create_renewal):
        ...
        self.assertIn("License Manager API error", str(context.exception))
    ...
    def test_process_input_gets_latest_stripe_subscription_id(self, mock_create_renewal):
        ...
        expected_renewal_id = UUID('00000000-0000-0000-0000-000000000789')
        renewal_record = SelfServiceSubscriptionRenewal.objects.get(
            checkout_intent=self.checkout_intent,
            subscription_plan_renewal_id=expected_renewal_id
        )
