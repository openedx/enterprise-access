...
from enterprise_access.apps.api_client import license_manager_client as lm_client
...
def cancel_all_future_plans(
    ...
    """
    client = lm_client.LicenseManagerApiClient()
...
class StripeEventHandler:
    ...
    def subscription_created(event: stripe.Event) -> None:
        ...
        """
        subscription = event.data.object
        logger.info('Enabling pending updates for created subscription %s', subscription.id)
        try:
    ...
    def subscription_updated(event: stripe.Event) -> None:
        ...
        if current_status == "canceled" and prior_status != "canceled":
            ...
            if trial_end:
                ...
                logger.warning("cancellation code branch reached, but handler not yet implemented.")
