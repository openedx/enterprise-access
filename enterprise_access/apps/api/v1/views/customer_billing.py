...
class CustomerBillingViewSet(viewsets.ViewSet):
    ...
    def stripe_webhook(self, request):
        ...
        * For a real production implementation we should implement event de-duplication:
        ...
        webhook_secret = settings.STRIPE_WEBHOOK_ENDPOINT_SECRET

        try:
            # Verify the signature and construct the event
            # Note: Authentication class already validated the signature,
            # but we need to reconstruct the event here to process it
            ...
            # Invalid signature (should not happen if authentication passed)
            return Response(
