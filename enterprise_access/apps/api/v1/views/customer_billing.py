...
class CustomerBillingViewSet(PermissionRequiredMixin, viewsets.ViewSet):
    """
    ...
    @extend_schema(
        ...
        request=serializers.BillingCreatePlanRequestSerializer,
    )
    ...
    def create_plan(self, request, *args, **kwargs):
        ...
        try:
            enterprise_customer_data = lms_client.get_enterprise_customer_data_by_slug(form_slug)
        except requests.exceptions.HTTPError:
            logger.info('No existing customer found for slug {form_slug}. Creating plan.')
        else:
            logger.warning('Existing customer found for slug {form_slug}. Cannot create plan.')
    ...
    @extend_schema(
        ...
        request=serializers.BillingCreatePlanRequestSerializer,
        responses={
            status.HTTP_201_CREATED: serializers.BillingCreatePlanResponseSerializer,
        },
    )
    @action(detail=True, methods=['get'])
    @permission_required(CUSTOMER_BILLING_READ_PERMISSION, fn=lambda request, pk: pk)
    def create_portal_session(self, request, pk=None, **kwargs):
        ...
        try:
            enterprise_customer_data = lms_client.get_enterprise_customer_data(uuid)
        except requests.exceptions.HTTPError:
    ...
    @action(detail=False, methods=['post'])
    @permission_required(CUSTOMER_BILLING_WEBHOOK_PERMISSION)
