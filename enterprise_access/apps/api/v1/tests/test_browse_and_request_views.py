...
class TestLearnerCreditRequestViewSet(BaseEnterpriseAccessTestCase):
    ...
    def test_approve_with_existing_errored_assignment(
        self, mock_get_and_cache_content_metadata, mock_catalog_contains,
        mock_content_metadata, mock_get_enterprise_uuid
    ):
        """
        Test successful approval of a learner credit request with an existing assignment in ERRORED state.
        """
        # Setup a request with an existing assignment in ERRORED state
        course_key = 'some-course-key'
        learner = self.user
        assignment_with_errored_state = LearnerContentAssignmentFactory(
            assignment_configuration=self.assignment_config,
        ...
        request_with_existing_assignment = LearnerCreditRequestFactory(
            enterprise_customer_uuid=self.enterprise_customer_uuid_1,
            user=learner,
            learner_credit_request_config=self.learner_credit_config,
            ...
            course_price=1000,
            state=SubsidyRequestStates.REQUESTED,
            assignment=assignment_with_errored_state
        )
        mock_get_enterprise_uuid.return_value = str(self.enterprise_customer_uuid_1)
        mock_content_metadata.return_value = {
            'content_key': course_key,
        ...
        data = {
            "enterprise_customer_uuid": str(self.enterprise_customer_uuid_1),
            "policy_uuid": str(self.policy.uuid),
            "learner_credit_request_uuid": str(request_with_existing_assignment.uuid)
        }
        response = self.client.post(url, data)
        assert response.status_code == status.HTTP_200_OK

        # Verify request was approved
        request_with_existing_assignment.refresh_from_db()
        assert request_with_existing_assignment.state == SubsidyRequestStates.APPROVED
        assert request_with_existing_assignment.reviewer == self.user

        # Verify assignment was created with correct fields
        assert request_with_existing_assignment.assignment == assignment_with_errored_state
        assert (
            request_with_existing_assignment.assignment.content_quantity ==
            request_with_existing_assignment.course_price * -1
        )
        assert request_with_existing_assignment.assignment.assignment_configuration == self.assignment_config
        assert request_with_existing_assignment.assignment.state == 'allocated'
        assert request_with_existing_assignment.assignment.learner_email == request_with_existing_assignment.user.email
        assert request_with_existing_assignment.assignment.content_key == request_with_existing_assignment.course_id

        # Verify RequestAction was created
        self.assertIsNotNone(
            request_with_existing_assignment.actions.get(
                learner_credit_request=request_with_existing_assignment,
                recent_action=get_action_choice(SubsidyRequestStates.APPROVED),
