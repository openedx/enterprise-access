...
class TestCheckoutIntentSynchronization(APITest):
    ...
    @mock.patch('enterprise_access.apps.provisioning.api.LicenseManagerApiClient')
    @mock.patch('enterprise_access.apps.provisioning.api.LmsApiClient')
    def test_checkout_intent_synchronized_on_success(self, mock_lms_api_client, mock_license_manager_client):
        """
        Test that a PAID checkout intent is linked to workflow and marked as FULFILLED on success.
        """
    ...
    def test_checkout_intent_wrong_state_ignored(
        ...
        """
        Test that checkout intents not in PAID state are ignored during synchronization.
        """
