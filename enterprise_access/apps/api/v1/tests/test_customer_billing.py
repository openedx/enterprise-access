...
class StripeWebhookTests(APITest):
    ...
    def test_webhook_fails_with_invalid_payload(self, mock_construct_event):
        """
        Test webhook endpoint handles invalid payload gracefully.
        """
        # First call in auth succeeds, second call in view fails
        mock_construct_event.side_effect = [
            {'id': 'evt_test'},  # Authentication passes
            ValueError('Invalid payload'),  # View processing fails
        ]
        ...
        self.assertIn('invalid', response.data.lower())
