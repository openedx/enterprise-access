...
from enterprise_access.constants import BRAZE_GROUPS_EMAIL_CAMPAIGNS, DAYS_TO_PURGE_PII
...
def get_braze_campaign_properties(membership_created_time, enterprise_customer_name, content_metadata, budget_end_date):
    """
    ...
    if current_date - datetime.strptime(membership_created_time, "%d %b %Y") == timedelta(days=BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FIRST_REMINDER.DAY):
        return {
            'braze_campaign_id': BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FIRST_NOTIFICATION.ID,
    ...
    if current_date - datetime.strptime(membership_created_time, "%d %b %Y") == timedelta(days=BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.SECOND_REMINDER.DAY):
        return {
            'braze_campaign_id': BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.SECOND_NOTIFICATION.ID,
            'braze_trigger_properties': {
                'catalog_content_count': len(content_metadata),
                'invitation_end_date': invitation_end_date,
                'budget_end_date': budget_end_date
    ...
    if current_date - datetime.strptime(membership_created_time, "%d %b %Y") == timedelta(days=BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.THIRD_REMINDER.DAY):
        return {
            'braze_campaign_id': BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.THIRD_NOTIFICATION.ID,
            'braze_trigger_properties': {
                'catalog_content_count': len(content_metadata),
                'invitation_end_date': invitation_end_date,
                'budget_end_date': budget_end_date
    ...
    if current_date - datetime.strptime(membership_created_time, "%d %b %Y") == timedelta(days=BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FOURTH_REMINDER.DAY):
        return {
            'braze_campaign_id': BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FOURTH_NOTIFICATION.ID,
            'braze_trigger_properties': {
                'enterprise_customer': enterprise_customer_name,
                'catalog_content_count': len(content_metadata),
                'invitation_end_date': invitation_end_date,
    ...
    if current_date - datetime.strptime(membership_created_time, "%d %b %Y") == timedelta(days=BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FINAL_REMINDER.DAY):
        return {
            'braze_campaign_id': BRAZE_GROUPS_EMAIL_CAMPAIGNS.AUTO_REMINDER.FINAL_NOTIFICATION.ID,
            'braze_trigger_properties': {
                'catalog_content_count': len(content_metadata),
                'invitation_end_date': invitation_end_date,
                'budget_end_date': budget_end_date
...
def get_subsidy_end_date(enterprise_customer):
    """
    Helper function to return policy end date for enterprise customer.
    """
    subsidies = SubsidyAccessPolicy.objects.filter(
        enterprise_customer_uuid=enterprise_customer.uuid,
        active=True
    )
    membership_group_uuid = enterprise_customer.group
    # list of policies []
    # loop through the list of policies 
    # find a policy with the same group uuid


@shared_task(base=LoggedTaskWithRetry)
def send_group_reminder_emails(pending_group_membership):
    """
    ...
    braze_client_instance = BrazeApiClient()
    enterprise_customer_name = pending_group_membership.group.enterprise_customer.name
    braze_trigger_properties = {}
    braze_trigger_properties['enterprise_customer_name'] = enterprise_customer_name
    pecu_email = pending_group_membership.pending_enterprise_customer_user.user_email
    ...
    content_metadata = []
    budget_end_date =  get_subsidy_end_date(pending_group_membership.group.uuid, pending_group_membership.group.enterprise_customer.uuid)
   
    braze_properties = get_braze_campaign_properties(pending_group_membership.created_at, enterprise_customer_name, content_metadata, budget_end_date)
    braze_client_instance.send_campaign_message(
...
@shared_task(base=LoggedTaskWithRetry)
def send_group_membership_invitation_notification(enterprise_customer, member):
    """
    Send braze email notification when member is invited to a group.
        
    Arguments:
        * enterprise_customer (dict)
        * learner (dict)
    """
    braze_client_instance = BrazeApiClient()
    enterprise_catalog_client = EnterpriseCatalogApiClient(member.enterprise_customer_user)
    braze_trigger_properties = {}
    braze_campaign_id = BRAZE_GROUPS_EMAIL_CAMPAIGNS.INVITATION_NOTIFICATION_ID
    ...
    braze_trigger_properties['enterprise_customer_name'] = enterprise_customer_name
    braze_trigger_properties['catalog_content_count'] = enterprise_catalog_client.get_catalog_content_count(enterprise_customer.enterprise_customer_catalogs.uuid)
    learner_email = member.enterprise_customer_user.user_email
    subsidy_end_date = ''
    braze_trigger_properties['budget_end_date'] = subsidy_end_date
    
    if member.enterprise_customer_user.user_id is None:
        learner_email = member.pending_enterprise_customer_user.user_email
        recipient = braze_client_instance.create_recipient_no_external_id(
            learner_email,
        )
        # We need an alias record to exist in Braze before
        ...
        recipient = braze_client_instance.create_recipient(
            ...
            user_id=member.enterprise_customer_user.user_id,
        )
