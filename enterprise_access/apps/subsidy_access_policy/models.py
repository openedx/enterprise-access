...
class AssignedLearnerCreditAccessPolicy(AssignedCreditPolicyMixin, SubsidyAccessPolicy):
    ...
    def validate_learner_credit_requests_allocation_prices(self, learner_credit_requests, all_metadata):
        ...
        for request in learner_credit_requests:
            if request.course_price < 0:
                request.failure_reason = 'Can only allocate non-negative course_price_cents'
    ...
    def can_allocate_all(self, learner_credit_requests):
        ...
        logger.info(f"all_content_keys: {get_and_cache_catalog_content_metadata(self.catalog_uuid, all_content_keys)}")
        all_metadata = {
            meta['key']: meta
