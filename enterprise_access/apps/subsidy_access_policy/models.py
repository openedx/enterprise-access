...
class SubsidyAccessPolicy(TimeStampedModel):
    ...
    def can_approve(self, learner_credit_requests):
        ...
        if not self.bnr_enabled:
            return {
                "valid_requests": [],
                "failed_requests_by_reason": {REASON_BNR_NOT_ENABLED: list(learner_credit_requests)}
            }
...
class AssignedLearnerCreditAccessPolicy(AssignedCreditPolicyMixin, SubsidyAccessPolicy):
    ...
    def validate_learner_credit_requests_allocation_prices(self, learner_credit_requests, all_metadata):
        ...
        for request in learner_credit_requests:
            ...
            # This check is now mostly for safety; existence should have been pre-validated.
            if not metadata or metadata.get('content_price') is None:
                request.failure_reason = f"Canonical price for {request.course_id} could not be found."
                failed_requests.append(request)
                continue

            canonical_price_cents = metadata['content_price']
    ...
    def can_allocate_all(self, learner_credit_requests):
        ...
        # inactive policy
        if not self.is_redemption_enabled:
            failed_requests_by_reason[REASON_POLICY_EXPIRED] = list(learner_credit_requests)
            return {"valid_requests": [], "failed_requests_by_reason": failed_requests_by_reason}

        # inactive subsidy
        if not self.is_subsidy_active:
            failed_requests_by_reason[REASON_SUBSIDY_EXPIRED] = list(learner_credit_requests)
            return {"valid_requests": [], "failed_requests_by_reason": failed_requests_by_reason}
        ...
        if self.content_would_exceed_limit(
            ...
            return {"valid_requests": [], "failed_requests_by_reason": {REASON_NOT_ENOUGH_VALUE_IN_SUBSIDY: valid_requests}}
        ...
        if self.content_would_exceed_limit(
            ...
            return {"valid_requests": [], "failed_requests_by_reason": {REASON_POLICY_SPEND_LIMIT_REACHED: valid_requests}}
