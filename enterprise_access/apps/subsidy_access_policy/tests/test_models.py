...
class SubsidyAccessPolicyTests(MockPolicyDependenciesMixin, TestCase):
    ...
    def test_per_learner_spend_policy_approve_method_assignment_allocated(self):
        """
        Test PerLearnerSpendCreditAccessPolicy.approve method calls assignments_api.allocate_assignments()
          with correct paramaters when a new assignment is created.
        """
        # set up assignment, configuration and allocate_assignments mock return value
        ...
        self.mock_assignments_api.allocate_assignments.return_value = {
            'updated': [],
            'created': [assignment],
            'no_change': [],
        }
        ...
        with patch(
            ...
            # assert that the assignments_api.allocate_assignments was called with correct parameters
            self.mock_assignments_api.allocate_assignments.assert_called_once_with(
                assignment_configuration,
                [test_learner_email],
                self.course_id,
                test_course_price,
                [self.lms_user_id],
                False,  # link_to_enterprise_user
                False   # send_notification_email
            )

    def test_per_learner_spend_policy_approve_existing_assignment_updated(self):
        """
        Test PerLearnerSpendCreditAccessPolicy.approve method calls assignments_api.allocate_assignments()
          with correct paramaters when a existing assignment is updated.
        """
        # set up assignment, configuration and allocate_assignments mock return value
        assignment_configuration = AssignmentConfiguration.objects.create()
        test_learner_email = 'test@email.com'
